...
async def async_setup_entry(hass, entry):
    async def update_charger(val):
        _LOGGER.warning(f"RATIO: Schrijven naar 16640 -> {val}A")
        try:
            ...
            _LOGGER.error(f"RATIO: Modbus fout: {e}")
    ...
    async_track_time_interval(hass, check_and_adjust, timedelta(seconds=10))
